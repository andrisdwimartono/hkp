...
class INFORMASIPEKERJAANBARU(Document):
	def validate(self):
		row = 0
	...
	def after_insert(self):
		...
		for d in self.pejabat:
			...
			notification_doc = {
				...
				"subject": "Informasi Pekerjaan Baru {0}".format(self.name),
...
def view_doc(docname):
	frappe.db.sql("UPDATE `tabProcess Rule2` SET status = 'Viewed' WHERE parent = '{0}' AND parenttype = 'Informasi Pekerjaan Baru' AND user = '{1}'".format(docname, frappe.session.user))