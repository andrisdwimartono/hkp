...
class KartuPemeliharaanAC(Document):
	...
	def autoname(self):
		...
		a = frappe.db.sql("""SELECT COUNT(*) cnt FROM `tabKartu Pemeliharaan AC` WHERE tanggal between '{0}' and '{1}'""".format("{0}-01-01".format(mydate.year), "{0}-12-31".format(mydate.year)), as_dict=1)
		if a:
			urut = a[0].cnt+1
			urut2 = ""
			if urut < 9:
				...
				urut2 = "{0}".format(urut)
			self.name = "{2}/IT/KPA/{0}/{1}".format(bulan, mydate.year-2000, urut2, get_department_abbr_by_session())
		else:
			self.name = "001/{2}/IT/KPA/{0}/{1}".format(bulan, mydate.year-2000, get_department_abbr_by_session())
