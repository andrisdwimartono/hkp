...
def get_data(filters=None):
	...
	if filters:
		...
		if filters.get("purchase_order"):
			whereSPB += " AND spb.purchase_order = '{0}'".format(filters.get("purchase_order"))
	return frappe.db.sql(
		...
		SELECT
			...
			CASE WHEN realisasi.total_paid IS NOT NULL THEN spb.grand_total/realisasi.total_paid*100 ELSE 0 END AS progress_achieved,
			COALESCE(realisasi.total_paid, 0) AS total_paid,
