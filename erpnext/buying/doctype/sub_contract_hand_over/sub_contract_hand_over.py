...
class SubContractHandOver(Document):
	def autoname(self):
		...
		a = frappe.db.sql("""SELECT COUNT(*) cnt FROM `tabSub Contract Hand Over` WHERE project = '{0}'""".format(self.project), as_dict=1)
		if a:
			urut = a[0].cnt+1
			urut2 = ""
			if urut < 9:
				...
				urut2 = "{0}".format(urut)
			self.name = "{2}/LOG-SPK/{3}/{0}/{1}".format("0{0}".format(mydate.month) if mydate.month < 10 else mydate.month, mydate.year-2000, urut2, self.project)
		else:
			self.name = "001/LOG-SPK/{2}/{0}/{1}".format("0{0}".format(mydate.month) if mydate.month < 10 else mydate.month, mydate.year-2000, self.project)
