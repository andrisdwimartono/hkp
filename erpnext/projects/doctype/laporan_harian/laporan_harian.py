...
class LaporanHarian(Document):
	def autoname(self):
		...
		a = frappe.db.sql("""SELECT COUNT(*) cnt FROM `tabLaporan Harian` WHERE name like '%/{2}/{0}/{1}'""".format(bulan, mydate.year, self.project), as_dict=1)
		if a:
			urut = a[0].cnt+1
			urut2 = ""
			if urut < 9:
