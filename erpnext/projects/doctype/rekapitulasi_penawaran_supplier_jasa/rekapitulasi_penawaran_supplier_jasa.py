...
class REKAPITULASIPENAWARANSUPPLIERJASA(Document):
	def autoname(self):
		...
		a = frappe.db.sql("""SELECT COUNT(*) cnt FROM `tabREKAPITULASI PENAWARAN SUPPLIER JASA` WHERE name like '%/{0}/{1}'""".format(bulan, mydate.year), as_dict=1)
		if a:
			urut = a[0].cnt+1
			urut2 = ""
			if urut < 9:
				...
				urut2 = "{0}".format(urut)
			self.name = "{2}/DKD/SUS/{0}/{1}".format(bulan, mydate.year, urut2)
		else:
			self.name = "01/DKD/SUS/{0}/{1}".format(bulan, mydate.year)
