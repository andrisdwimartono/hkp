...
class LAPORANTINDAKANPERBAIKANDANPENCEGAHAN(Document):
	def validate(self):
		...
		if self.workflow_state == "Diperiksa":
			if self.diperiksa_oleh_user and ltbc and ltbc[0].workflow_state == "Disetujui":
				...
				frappe.throw("User pemeriksa belum ada atau status saat ini bukan Disetujui")
		...
		if self.workflow_state == "Disetujui":
			if self.disetujui_oleh_user and ltbc and ltbc[0].workflow_state == "Dibuat":
				...
				frappe.throw("User penyetuju belum ada atau status saat ini bukan Dibuat")
