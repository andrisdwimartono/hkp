	...
	def validate(self):
		...
		for li in lists:
			...
			dts = frappe.db.sql("""SELECT b.name, h.volume_cumulative_last_week FROM `tabSub Contract Weekly Progress` a
				INNER JOIN `tabSub Contract Weekly Progress Detail3` b ON b.parent = a.name
				LEFT JOIN (SELECT g.job_detail, SUM(g.vol_this_week) volume_cumulative_last_week FROM `tabSub Contract Weekly Progress` f
					INNER JOIN `tabSub Contract Weekly Progress Detail3` g ON g.parent = f.name
			...
			for dt in dts:
				frappe.db.sql("""UPDATE `tabSub Contract Weekly Progress Detail3` 
...
def check_last_week(master_item_pekerjaan = None, sub_contract_hand_over = None, week = None):
	if sub_contract_hand_over and week:
		x = frappe.db.sql("""
			SELECT x.item_pekerjaan job_detail, b.*, e.tot_vol_this_week FROM `tabMaster Item Pekerjaan Detail` x
			...
			LEFT JOIN `tabSub Contract Weekly Progress Detail3` b ON b.parent = c.name
			LEFT JOIN (SELECT  e.job_detail, SUM(e.vol_this_week) tot_vol_this_week FROM `tabSub Contract Weekly Progress Detail3` e
		...
		if x:
       	...
       	SELECT x.item_pekerjaan job_detail, b.*, 0 tot_vol_this_week, x.uom, x.vol_kontrak FROM `tabMaster Item Pekerjaan Detail` x
...
def get_sub_contract_weekly_progress_detail(master_sub_contract_weekly_progress, week, sub_contract_hand_over):
	if sub_contract_hand_over and week:
		...
		INNER JOIN `tabMaster Sub Contract Weekly Progress Detail3` x ON x.parent = a.master_sub_contract_weekly_progress
		LEFT JOIN `tabSub Contract Weekly Progress Detail3` b ON b.parent = a.name AND x.job_detail = b.job_detail
		LEFT JOIN (SELECT d.* FROM `tabSub Contract Weekly Progress` c
			INNER JOIN `tabSub Contract Weekly Progress Detail3` d ON d.parent = c.name
		...
		LEFT JOIN (SELECT j.job_detail, SUM(j.vol_this_week) volume_cumulative_thist_week FROM `tabSub Contract Weekly Progress` i
			INNER JOIN `tabSub Contract Weekly Progress Detail3` j ON j.parent = i.name
		...
		if a:
			...
			INNER JOIN `tabMaster Sub Contract Weekly Progress Detail3` x ON x.parent = a.master_sub_contract_weekly_progress
			LEFT JOIN `tabSub Contract Weekly Progress Detail3` b ON b.parent = a.name
			LEFT JOIN (SELECT g.job_detail, SUM(g.vol_this_week) volume_cumulative_last_week FROM `tabSub Contract Weekly Progress` f
				INNER JOIN `tabSub Contract Weekly Progress Detail3` g ON g.parent = f.name
				WHERE f.week <= '{2}' AND f.sub_contract_hand_over = '{1}'
				GROUP BY g.job_detail
...
def saving(name, sub_contract_hand_over, project, sub_contract, job_name, posting_date, week, contractor_name, project_name, pic_name, budget_amount, sub_contract_weekly_progress, sub_contract_weekly_progress_last_week, sub_contract_weekly_progress_detail):
	...
	if nm:
		...
		# for scwpd in scwp.sub_contract_weekly_progress_detail:
			# frappe.delete_doc("Sub Contract Weekly Progress Detail3", scwpd.name)
		for n in aaa:
